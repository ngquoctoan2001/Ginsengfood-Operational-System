...
When an agent starts a dev server or Playwright web server, track the PID and stop it before final response:

```powershell
.\tools\agent\Stop-AgentOwnedProcesses.ps1 -IncludeDescendants
```

Never kill every `node`, `npm`, Vite, or Playwright process by name; those may belong to the owner.
