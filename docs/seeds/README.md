...
| Owner-pending inactive `trade_item` rows | 20 |
...
| GTIN/GS1 | Chưa có owner-approved production values; seed tạo 20 inactive owner-pending `trade_item` và đúng một active `TEST_ONLY_DEV_FIXTURE` `trade_item_gtin`/`packaging_trade_item_map` cho local validation. Fixture này không phải GTIN thật. |
