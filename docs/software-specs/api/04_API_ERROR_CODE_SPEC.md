...
| `SUPPLIER_INGREDIENT_NOT_ALLOWED`           | 422     | `(supplier_id, ingredient_id)` không có trong `op_supplier_ingredient` (HL-SUP-002 / HL-SUP-013 / BR-M03A-004 / BR-M06-006) | Block receipt; gợi ý liên hệ procurement bổ sung mapping. |
...
| `WEAK_PASSWORD`                             | 422     | Password supplier user không đạt policy HL-SUP-008 (min 12 ký tự, mixed case + digit + symbol)                              | Show password rule; require stronger password.            |
...
| `SUPPLIER_EDIT_LOCKED_AFTER_CONFIRMED`      | 409     | Edit receipt sau supplier confirm hoặc supplier-submit ở `WAITING_DELIVERY` (HL-SUP-002 / BR-M06-007)                       | Block edit; gợi ý tạo correction/feedback.                |
...
| `RECEIPT_LOCKED_AFTER_RECEIVE`              | 409     | Edit quantity/ingredient sau khi receipt `RECEIVED_PENDING_QC` (HL-SUP-004 / BR-M06-009)                                    | Block edit; chỉ cho accept/reject/return/close.           |
| `SUPPLIER_EVIDENCE_REQUIRED`                | 422     | Receive khi policy yêu cầu evidence trước nhưng chưa có evidence `CLEAN` (HL-SUP-006 / BR-M06-010)                          | Block receive; focus evidence uploader.                   |
| `LOT_QUANTITY_EXCEEDS_RECEIVED`             | 422     | Sum(lot quantity per line) vượt `received_quantity` của line (HL-SUP-014 / BR-M06-012)                                      | Block receive; điều chỉnh lot quantity.                   |
| `RECEIPT_QUANTITY_INVARIANT_FAILED`         | 422     | Sum(received + rejected + returned) per line ≠ expected khi close (HL-SUP-014 / BR-M06-013)                                 | Block close; show line invariant detail.                  |
