...
| Precondition         | Supplier `ACTIVE`; supplier user link tồn tại; ingredient nằm trong `op_supplier_ingredient_allowed`; mật khẩu hợp `HL-SUP-007`.                                                                                                                                                                                                                                                                                                                                                                           |
| Main flow            | 1. Supplier login `/api/supplier/auth/login`. 2. Supplier `POST /api/supplier/intakes` tạo intake `DRAFT` (axis A `SUPPLIER_SUBMITTED`, axis B `PENDING_SUPPLIER_CONFIRM`). 3. Supplier upload evidence; chờ scan `CLEAN`. 4. Supplier `confirm` -> axis A `SUPPLIER_CONFIRMED`, axis B `PENDING_RECEIVE`. 5. Admin `acknowledge` -> axis A `COMPANY_ACKNOWLEDGED`. 6. Admin `receive` -> axis B `RECEIVED`. 7. Line accept/reject/return; close intake -> axis A `COLLABORATION_CLOSED`, axis B `CLOSED`. |
...
| Postcondition        | Single source of truth `op_raw_material_receipt` (không tạo `op_supplier_delivery` song song); raw lot generated ở axis B `RECEIVED`; trace link supplier -> intake -> lot.                                                                                                                                                                                                                                                                                                                                |
| API/UI               | `/api/supplier/intakes/*`, `/api/supplier/intakes/{id}/evidence`, `/api/supplier/intakes/{id}/confirm                                                                                                                                                                                                                                                                                                                                                                                                      | decline`; `/api/admin/raw-material/intakes/{id}/acknowledge | receive | lines/{lineId}/accept | reject | return | close | feedback`; SCR-SUP-LOGIN, SCR-SUP-PORTAL-INTAKES, SCR-SUP-PORTAL-EVIDENCE, SCR-RAW-INTAKE-DETAIL, SCR-RAW-RECEIVE, SCR-RAW-LINE-QC |
