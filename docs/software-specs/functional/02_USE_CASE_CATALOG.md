...
- [UC-02A - Supplier Collaboration pre-receipt qua Supplier Portal](#uc-02a---supplier-collaboration-pre-receipt-qua-supplier-portal)
...
## UC-02A - Supplier Collaboration pre-receipt qua Supplier Portal

| Field                | Nội dung                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Actor                | `R-SUPPLIER` (portal), `R-WH-RAW`, `R-OPS-MGR` (admin acknowledge/receive)                                                                                                                                                                                                                                                                                                                                                                                                                |
| Module               | M03A Supplier Management, M06 Raw Material (collab axis A)                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Precondition         | Supplier `ACTIVE`; supplier user link tồn tại; ingredient nằm trong `op_supplier_ingredient`; mật khẩu hợp `HL-SUP-008`.                                                                                                                                                                                                                                                                                                                                                                  |
| Main flow            | 1. Supplier login `/api/supplier/auth/login`. 2. Supplier `POST /api/supplier/raw-material/intakes` tạo intake axis A `SUPPLIER_SUBMITTED -> SUPPLIER_CONFIRMED`, axis B `WAITING_DELIVERY`. 3. Supplier upload evidence; chờ scan `CLEAN`. 4. Admin review receipt detail; nếu axis A `SUPPLIER_CONFIRMED` và axis B `WAITING_DELIVERY` thì `receive` -> axis B `RECEIVED_PENDING_QC`. 5. Line accept/reject/return; close intake -> axis B `CLOSED` và axis A giữ `SUPPLIER_CONFIRMED`. |
| Alternate/error flow | Supplier decline -> axis A `SUPPLIER_DECLINED`, axis B `CANCELLED` (EX-SUP-DECLINE). Ingredient ngoài allowlist -> reject `SUPPLIER_INGREDIENT_NOT_ALLOWED`. Truy cập intake supplier khác -> `403 SUPPLIER_SCOPE_VIOLATION`. Receive khi axis A chưa `SUPPLIER_CONFIRMED` -> reject. Sum invariant vi phạm -> reject (UI-VAL-SUP-009).                                                                                                                                                   |
| Postcondition        | Single source of truth `op_raw_material_receipt` (không tạo `op_supplier_delivery` song song); raw lot generated ở axis B `RECEIVED_PENDING_QC`; trace link supplier -> intake -> lot.                                                                                                                                                                                                                                                                                                    |
| API/UI               | Supplier portal: `/api/supplier/raw-material/intakes/*`, `/api/supplier/raw-material/intakes/{id}/evidence`, `/api/supplier/raw-material/intakes/{id}/confirm`, `/api/supplier/raw-material/intakes/{id}/decline`. Admin: `/api/admin/raw-material/intakes/{id}/receive`, `/lines/{lineId}/accept`, `/reject`, `/return`, `/close`, `/feedback`. UI: SCR-SUP-LOGIN, SCR-SUP-PORTAL-INTAKES, SCR-SUP-PORTAL-EVIDENCE, SCR-RAW-INTAKE-DETAIL, SCR-RAW-RECEIVE, SCR-RAW-LINE-QC.             |
| Acceptance           | AC-SUP-001..AC-SUP-015                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| Alternate/error flow | Có research baseline token trong operational data -> reject; thiếu group/ingredient bắt buộc -> reject; active overlap cùng `(sku_id, formula_kind)` -> reject.                |
| Postcondition        | SKU có active approved recipe G1 `PILOT_PERCENT_BASED`; future G2 `FIXED_QUANTITY_BATCH` có thể coexist theo `(sku_id, formula_kind)`.                                         |
...
| Main flow            | 1. Tạo packaging job PACKET/BOX/CARTON với optional `carton_requested` (CARTON yêu cầu `op_trade_item.carton_enabled = true` và `boxes_per_carton > 0`, nếu không reject `CARTON_PACKAGING_NOT_CONFIGURED`). 2. Validate trade item identifier (`identifier_type` + `identifier_value`) unique theo package level. 3. Generate QR (BOX/CARTON; PACKET không có QR). 4. Queue print. 5. Print success -> QR `PRINTED`. 6. Nếu reprint, request reason và link original. |
...
| Main flow            | 1. Open incident/recall case. 2. Run impact analysis và lưu snapshot. 3. Apply hold/sale lock. 4. Ghi notification reference. 5. Theo dõi recovery. 6. Ghi disposition/CAPA. 7. Upload CAPA evidence metadata qua storage adapter và chờ `scan_status = CLEAN`. 8. Close thường khi đủ điều kiện hoặc close với residual risk khi có residual note/approval. |
| Alternate/error flow | Re-run impact tạo snapshot mới; close khi recovery open -> reject; close khi thiếu clean CAPA evidence -> `EVIDENCE_REQUIRED`/`EVIDENCE_SCAN_PENDING`/`EVIDENCE_SCAN_FAILED`/`EVIDENCE_MALWARE_DETECTED`; close residual risk thiếu `residual_note` -> `RECALL_RESIDUAL_RISK_NOTE_REQUIRED`; cancel chỉ khi chưa có downstream action hoặc theo policy.      |
| Postcondition        | Recall có timeline/audit, exposure snapshot, recovery/disposition/CAPA, CAPA evidence metadata sạch và trạng thái `CLOSED` hoặc `CLOSED_WITH_RESIDUAL_RISK`.                                                                                                                                                                                                 |
| API/UI               | `/api/admin/incidents/*`, `/api/admin/recall/*`, `/api/admin/recall/capas/{capaId}/evidence`; Recall Case Management, Recovery / CAPA                                                                                                                                                                                                                        |
