...
- Recipe coexistence G1 PILOT_PERCENT_BASED + G2 FIXED_QUANTITY_BATCH (active per `(sku_id, formula_kind)`) cho 20 SKU baseline. G1 không phải baseline duy nhất.
...
| SKU/recipe governance   | 20 SKU baseline, ingredient master, formula coexistence G1 PILOT_PERCENT_BASED + G2 FIXED_QUANTITY_BATCH active per `(sku_id, formula_kind)`, future versions. | M04                |
...
→ Production order with formula snapshot (G1 PILOT hoặc G2 FIXED theo `op_sku_operational_config.preferred_formula_kind`)
...
- 20 SKU baseline không phải giới hạn vĩnh viễn. Formula model coexistence G1 PILOT_PERCENT_BASED + G2 FIXED_QUANTITY_BATCH active đồng thời per `(sku_id, formula_kind)` (xem `01_SOURCE_INDEX.md §7` và `database/04_ENUM_REFERENCE.md`).
