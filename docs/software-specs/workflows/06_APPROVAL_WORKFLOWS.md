...
| Nguyên tắc                | Quy tắc                                                                                                                                                  |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Separation of duties      | Người tạo và người duyệt nên tách vai trò với các hành động rủi ro cao; nếu phase đầu chưa đủ dual approval thì ghi accepted-risk trong handoff và không bypass audit. |
...
| AP-REC-002             | Recipe activation                     | QA Manager / authorized role      | Authorized activator                                        | `APPROVED`                        | `APPROVED -> ACTIVE_OPERATIONAL`                            | N/A                                                                         | `POST /api/admin/recipes/{id}/activate`                                                                       | SCR-RECIPE                                     | No active overlap for same `(sku_id, formula_kind)`, effective date valid; G1 PILOT and G2 FIXED may coexist for same SKU                                                                                                                                                                                                                              | `RECIPE_ACTIVATED`                                             | TC-UI-REC-003          |
| AP-PO-001              | Production order approval             | Production Planner                | Production Manager                                          | `OPEN`                            | `OPEN -> APPROVED`                                          | `OPEN -> CANCELLED/REJECTED` by cancel/reject command family                 | `POST /api/admin/production/orders/{id}/approve`                                                              | SCR-PROD-ORDER-DETAIL                          | Snapshot complete, SKU active, recipe active, planned qty valid                                                                                                                                                                                                                                                                                         | `PRODUCTION_ORDER_APPROVED`                                    | TC-UI-PO-001           |
...
| AP-LOT-001             | Raw lot mark-ready approval           | QA Inspector / Warehouse Operator | QA Manager / Warehouse Manager                              | `lot_status = IN_QC` + `lot_qc_status = QC_PASS` | `IN_QC -> READY_FOR_PRODUCTION`                             | `IN_QC -> ON_HOLD/QUARANTINED`                                               | `POST /api/admin/raw-material/lots/{lotId}/readiness`                                                        | SCR-LOT-READINESS                              | QC result `QC_PASS`, source valid, balance available, no active hold/reject/quarantine/expiry, `RAW_LOT_MARK_READY` permission; `QC_PASSED_WAITING_READY` is derived UI state, not persisted `lot_status`                                                                                                                                                  | `RAW_LOT_READY_FOR_PRODUCTION` / `RAW_LOT_MARK_READY_REJECTED` | TC-M06-RM-005          |
...
| AP-ADJ-001             | Inventory adjustment approval         | Warehouse Manager                 | Admin / Finance / Warehouse Manager per adjustment tier     | `PENDING_APPROVAL`                | `PENDING_APPROVAL -> APPROVED/APPLIED`                      | `PENDING_APPROVAL -> REJECTED`                                              | `POST /api/admin/inventory/adjustments`; approve/reject via `/api/admin/approvals/{approvalRequestId}/approve` or `/reject` | SCR-INVENTORY-ADJUSTMENTS                      | Reason, attachment/evidence if required, balance impact                                                                                                                                                                                                                                                                                                 | `INVENTORY_ADJUSTMENT_APPROVED`                                | TC-UI-ADJ-001          |
...
| AP-MISA-001            | MISA mapping/reconcile approval       | Integration Operator              | Admin / Finance Viewer                                      | `FAILED_NEEDS_REVIEW`             | `FAILED_NEEDS_REVIEW -> RECONCILED`                         | Remain failed                                                               | `POST /api/admin/integrations/misa/sync-events/{id}/reconcile`                                                | SCR-MISA-RECONCILE                             | Mapping or mismatch reason                                                                                                                                                                                                                                                                                                                              | `MISA_RECONCILED`                                              | TC-UI-RECNC-001        |
| AP-OVR-001             | Break-glass override                  | Authorized operator               | Admin + required dual approver                              | Any blocked state                 | Controlled override transition                              | Reject override                                                             | `POST /api/admin/governance/overrides`; `POST /api/admin/governance/overrides/{overrideRequestId}/approve`                         | Relevant screen                                | Reason, scope, dual approval, expiry timestamp <= 15 minutes, no public policy bypass                                                                                                                                                                                                                                                                   | `OVERRIDE_APPROVED` / `OVERRIDE_REJECTED` / `OVERRIDE_EXPIRED` | TC-EXC-OVR-001         |
| AP-SUP-CONFIRM         | Supplier confirm pre-receipt          | Supplier user (R-SUPPLIER)        | N/A (self-service confirm); company chỉ nhận event          | axis A `PENDING_SUPPLIER_CONFIRMATION` | `-> SUPPLIER_CONFIRMED` (axis A) + axis B `WAITING_DELIVERY` | `-> SUPPLIER_DECLINED` (axis A) + axis B `CANCELLED` (đường EX-SUP-DECLINE) | `POST /api/supplier/raw-material/intakes/{id}/confirm` / `.../decline`                                        | SCR-SUP-PORTAL-INTAKES                         | Supplier ACTIVE; ingredient nằm allowlist; reason bắt buộc khi decline; sau confirm lock chỉnh sửa supplier-side                                                                                                                                                                                                                                        | `SUPPLIER_INTAKE_CONFIRMED` / `SUPPLIER_INTAKE_DECLINED`       | AC-SUP-006, AC-SUP-007 |
| AP-SUP-EVIDENCE-REVIEW | Supplier evidence review (`OD-M06-SUP-EVIDENCE-REVIEW`, HL-SUP-005/006/016) | Supplier user (R-SUPPLIER)        | QA Inspector / Warehouse Operator                           | evidence `PENDING_SCAN`/`CLEAN`   | `evidence.scan_status = CLEAN` cho phép confirm/receive/close | `evidence.scan_status ∈ {INFECTED, SCAN_FAILED}` chặn confirm/receive/close | `POST /api/admin/raw-material/intakes/{id}/evidence`; `POST /api/supplier/raw-material/intakes/{id}/evidence` | SCR-RAW-INTAKE-DETAIL, SCR-SUP-PORTAL-EVIDENCE | Mime/size theo allowlist; scan worker chạy async; chỉ evidence CLEAN mới satisfy evidence policy; evidence không lưu blob trong PostgreSQL; failed/malware evidence cần upload mới hoặc retry theo policy                                                                                                                                                    | `SUPPLIER_EVIDENCE_UPLOADED`, `SUPPLIER_EVIDENCE_SCAN_RESULT`  | TC-HL-SUP-016          |
| AP-M03A-SUP-SUSPEND    | Supplier suspend (`OD-M03A-SUP-SUSPEND`) | Supplier Manager / Ops Manager   | Ops Manager hoặc Admin                                      | supplier `ACTIVE`                 | `ACTIVE -> SUSPENDED`                                      | Remain `ACTIVE`                                                             | `POST /api/admin/suppliers/{id}/suspend`                                                                    | SCR-SUPPLIER-LIST, SCR-SUPPLIER-DETAIL         | Reason required; block/require override if supplier has open receipt after `RECEIVED_PENDING_QC`; suspend blocks supplier login/commands and new receipt submit; audit actor/reason | `SUPPLIER_SUSPENDED`                                           | TC-M03A-SUP-SUSPEND-001 |
| AP-M03A-SUP-REACTIVATE | Supplier reactivate (`OD-M03A-SUP-REACTIVATE`) | Supplier Manager / Ops Manager   | Ops Manager hoặc Admin                                      | supplier `SUSPENDED`              | `SUSPENDED -> ACTIVE`                                      | Remain `SUSPENDED`                                                          | `POST /api/admin/suppliers/{id}/reactivate`                                                                 | SCR-SUPPLIER-LIST, SCR-SUPPLIER-DETAIL         | Reason required; supplier profile, allowlist and user link must be valid; reactivation does not reopen declined/cancelled receipts | `SUPPLIER_REACTIVATED`                                         | TC-M03A-SUP-REACTIVATE-001 |
...
- Activation creates only one active operational version per `(sku_id, formula_kind)` at a time; G1 PILOT and G2 FIXED may coexist for the same SKU.
