...
|   2A | Supplier collaboration pre-receipt (PURCHASED) | M06/M03A | Supplier user (R-SUPPLIER) hoặc Warehouse Operator | `POST /api/supplier/raw-material/intakes` (NCC tự khai báo) hoặc `POST /api/admin/raw-material/intakes` rồi `POST /api/admin/raw-material/intakes/{id}/request-supplier-confirm`; supplier `POST /api/supplier/raw-material/intakes/{id}/confirm | decline`; evidence `POST .../evidence` | SCR-SUP-PORTAL-INTAKES, SCR-SUP-PORTAL-EVIDENCE, SCR-RAW-INTAKE-DETAIL                                           | `op_raw_material_receipt` (axis A `supplier_collaboration_status`, axis B `raw_receipt_status`), `op_raw_material_receipt_evidence`, `op_raw_material_receipt_feedback` | Receipt đạt axis A `SUPPLIER_CONFIRMED` (hoặc `COMPANY_ACKNOWLEDGED`) + axis B `PENDING_RECEIVE`                              | Supplier ACTIVE; ingredient nằm trong allowlist; evidence CLEAN nếu HL-SUP-009 yêu cầu; SUPPLIER_DECLINED → đường EX-SUP-DECLINE và CANCELLED |
