...
| TC-SEED-004 | M04      | No active/approved operational formula uses forbidden token; PILOT seed phải có anchor + SUM ratio đúng | Seed chain đã chạy        | Query operational recipe table for `formula_version='G0'` and active/approved status; chạy SV-019, SV-020 trên seed PILOT | Zero rows G0 active/approved; SV-019/SV-020 PASS (anchor đúng + SUM ∈ `[99.95, 100.05]`); validation fails nếu vi phạm                                                                                      | Recipe seed                | P0       | REQ-M04-004, REQ-M04-006, REQ-M04-007, REQ-NFR-004 |
...
| TC-SEED-015 | M06/M08  | Raw lot status and QC status support distinct production readiness gate                                 | Seed/ref enum initialized | Query raw lot status and raw lot QC status enum/state machine config                                                      | `lot_status` có `PENDING_QC`, `READY_FOR_PRODUCTION`; `lot_qc_status` (hoặc impl state machine) có `QC_PASS`, `QC_HOLD`, `QC_REJECT`; không có `QC_PASS` trong `lot_status` và không có alias `READY` mơ hồ | Raw lot status enum/config | P0       | REQ-M06-004, ADR-011                               |
| TC-SEED-016 | M03A/M06 | Supplier portal seed supports purchased intake smoke                                                    | Seed chain đã chạy        | Query `R-SUPPLIER`, `op_supplier`, `op_supplier_user_link`, `op_supplier_ingredient`                                      | `SUP_DEV_001`/`SUP_DEV_002` active; each has one supplier user; `R-SUPPLIER` has only `supplier.*` scoped permissions; allowlist mappings active/effective                                                  | Supplier seed              | P0       | REQ-SUP-COLLAB-010, REQ-SUP-COLLAB-011             |
...
```sql
-- SV-007: raw lot status and lot QC status (separate fields per ADR-011)
-- lot_status gate values (readiness + lifecycle)
SELECT status_code
FROM ref_lot_status
...
-- lot_qc_status values (QC decision result, NOT lot_status)
-- Validate via state machine config or enum table; QC_PASS must NOT be in ref_lot_status
SELECT status_code
FROM ref_lot_qc_status  -- or equivalent state machine config table
WHERE status_code IN (
  'QC_PASS',
  'QC_HOLD',
  'QC_REJECT'
