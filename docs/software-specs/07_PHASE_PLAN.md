...
| CODE02 | P0 | Raw Material Intake + Lot + Incoming QC | M06, M05, M11, M16 | CODE01 |
| CODE03 | P0 | Manufacturing Execution + Batch + Genealogy Foundation | M04, M07, M08, M11, M12, M16 | CODE01, CODE02, MX-GATE-G1 |
...
| CODE16 | P2 | Data Retention + Archival + Restore / Archive Search Boundary | M01, M11, M12, M13, M14, M15 | OD-12, OD-13 |
...
| Only G1 operational baseline | Không seed/use baseline nghiên cứu trong operational flow | prompt gốc, `SRC-LOCK5` |
...
| CODE04 | Khóa đóng gói/in/QR | Packaging job/unit, print job/log, QR registry, trade item/GTIN | Packaging, print, QR, trade item/GTIN | `/api/admin/packaging/*`, `/printing/*`, `/qr/*`, `/trade-items/*` | Packaging job, print queue, QR lifecycle, GTIN config | GTIN fixture `TEST_ONLY_DEV_FIXTURE`, print templates | QR lifecycle API test, reprint audit test, missing GTIN block test | QR đủ 6 state; reprint link original; commercial print cần GTIN | GTIN thật chưa có, dùng fixture |
...
| CODE12 | Khóa device/printer boundary | Printer/scanner adapter, heartbeat, callback, incident bridge | Device, heartbeat, callback log | `/api/admin/devices/*`, `/api/admin/printers/*`, callbacks | Device/printer console | Device fixture | Callback/heartbeat/failure tests | Device không access DB và không bypass QC/release | OD-17 printer model/driver |
...
| CODE16 | Khóa retention/archive/restore | Retention policy, archive/export, restore drill, archive search boundary | Retention policy, archive index/log | `/api/admin/retention/*`, `/api/admin/archive/*` | Retention/archive admin | Retention policy seed sau owner | Restore drill, archive search, retention tests | RPO/RTO + retention owner quyết định xong | OD-12, OD-13 open |
