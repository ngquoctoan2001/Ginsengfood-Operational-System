...
| Supplier collab (M03A) | `auth_role`, `auth_permission`, `role_action_permission`, `op_supplier`, `op_supplier_user` (hoặc `auth_user` extension), `op_supplier_ingredient` | Seed role `R_SUPPLIER` + namespace permission `supplier.read`, `supplier.receipt.create`, `supplier.receipt.submit`, `supplier.receipt.confirm`, `supplier.receipt.decline-confirmation`, `supplier.receipt.evidence.upload`, `supplier.receipt.cancel-draft`. Seed ít nhất 2 supplier dev fixture (`SUP_DEV_001`, `SUP_DEV_002`) với 1 supplier user mỗi supplier (mật khẩu tạm hash, đánh dấu `is_dev_fixture = true`). Seed `op_supplier_ingredient` mapping cho ingredient `PURCHASED` baseline (xem Section 4A). |
...
| SV-SUP-001    | Role `R_SUPPLIER` tồn tại và có đủ permission `supplier.read`, `supplier.receipt.create`, `supplier.receipt.submit`, `supplier.receipt.confirm`, `supplier.receipt.decline-confirmation`, `supplier.receipt.evidence.upload`, `supplier.receipt.cancel-draft`; KHÔNG có quyền nội bộ như `raw_intake.read`, `inventory.*`, `recipe.*`, `misa.*`, `trace.internal.*`. |
...
| SV-SUP-005    | Chạy seed chain 2 lần liên tiếp không sinh duplicate `op_supplier_ingredient`, `auth_role.R_SUPPLIER`, `role_action_permission` cho `supplier.*`.                                                                                                                                                                                                                    |
