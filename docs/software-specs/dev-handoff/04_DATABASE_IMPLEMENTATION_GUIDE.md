...
| Audit/history | `audit_log`, `state_transition_log`, `op_qr_state_history`, evidence metadata tables | Append-only; actor/reason/time required; evidence scan status may transition only to terminal scan result |
...
| Evidence metadata | `op_source_origin_evidence` and `op_recall_capa_evidence` require MIME/size allowlist, `evidence_scan_status`, storage URI/key, hash where available; DB stores metadata only |
...
Data repair for these tables requires owner-approved migration/runbook and must create evidence. Evidence metadata allows only controlled `scan_status` transition from `PENDING_SCAN` to `CLEAN`, `INFECTED` or `SCAN_FAILED`.
