...
Scaffold readiness note: current G1 CSV is shaped as `PILOT_PERCENT_BASED`. `ratio_percent` is normalized per recipe to SUM approximately 100, `is_anchor` is derived from the largest source quantity line, and header anchor fields mirror that line. `quantity_per_batch_400` remains in the CSV as source quantity evidence for owner review, not as the PILOT calculation basis.
...
| `batch_size_standard`      | numeric      | `400` cho FIXED baseline; PILOT PHẢI NULL (loader CẤM điền 400 cho PILOT header — G1-IMP-004).                                       |
...
| `quantity_per_batch_400` | numeric      | Bắt buộc `> 0` khi `formula_kind = FIXED_QUANTITY_BATCH`. PHẢI NULL khi PILOT; CSV loader CẤM điền giá trị 400 hoặc bất kỳ số nào cho PILOT line (G1-IMP-004). |
...
| G1-IMP-004 | Không import PILOT line có `ratio_percent <= 0`; không import FIXED line có `quantity_per_batch_400 <= 0`. PILOT line `quantity_per_batch_400` PHẢI NULL (loader CẤM điền 400 hoặc bất kỳ số nào); FIXED line `ratio_percent` được phép NULL (tham khảo). PILOT recipe `SUM(ratio_percent) ∈ [99.95, 100.05]` và đúng 1 line `is_anchor = true` trùng `anchor_ingredient_code`. PILOT header `batch_size_standard` PHẢI NULL.                                                                                         |
...
| G1-IMP-008 | For this scaffold-ready G1 seed, `quantity_per_batch_400` is retained as source evidence while `formula_kind = PILOT_PERCENT_BASED`; loader/tests must use normalized `ratio_percent` + anchor metadata for PILOT calculations and keep `owner_review_required = true`.                                                                                                                                                                                                                                               |
