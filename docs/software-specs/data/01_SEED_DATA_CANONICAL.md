...
| `csv/roles_permissions.csv`      |                    Starter RBAC action seed |             101 | READY_FOR_PRODUCTION_FREEZE, bao gồm `RAW_LOT_MARK_READY`, `BATCH_RELEASE_REVOKE`, `QR_GENERATE`, `QR_REPRINT`, `QR_VOID`, `DEVICE_CALLBACK`, M03A supplier admin permissions và `R-SUPPLIER` scoped permissions |
| `csv/suppliers.csv`              |                   M03A supplier dev fixture |               2 | DEV_TEST_ONLY; production supplier thật import riêng                                |
| `csv/supplier_users.csv`         |                Supplier portal user fixture |               2 | DEV_TEST_ONLY; no plaintext password                                                |
| `csv/op_supplier_ingredient.csv` |               Supplier ingredient allowlist |               3 | DEV_TEST_ONLY; mapping cần owner review trước production                            |
| `csv/warehouses_locations.csv`   | Kho nguyên liệu và kho thành phẩm tối thiểu | 4 location rows | READY                                                                               |
| `csv/source_origin_fixture.csv`  |         SELF_GROWN/PURCHASED source fixture |               2 | DEV_TEST_ONLY; production source/supplier thật import riêng                         |
| `csv/gtin_fixture.csv`           |     BOX/CARTON fake GTIN fixture cho 20 SKU |              40 | DEV_TEST_ONLY; production print phải block `is_test_fixture=true`                   |
| `csv/event_schema_registry.csv`  |                       Event schema baseline |              29 | READY_FOR_PRODUCTION_FREEZE, gồm supplier collaboration events                      |
| `csv/ui_registry_fixture.csv`    |             Minimum screen registry M01-M16 |              23 | DEV_TEST_ONLY; production screen/action registry review trước go-live               |
| `csv/misa_mapping_fixture.csv`   |               Mapping MISA dev/test fixture |               5 | DEV_TEST_ONLY; không phải production credential                                     |
...
|    03 | `suppliers.csv`              | `op_supplier`                                                       | auth schema                                  |
|    04 | `supplier_users.csv`         | `auth_user`/`op_supplier_user_link`                                 | supplier, auth schema                        |
|    05 | `warehouses_locations.csv`   | `op_warehouse`, `op_warehouse_location`                             | UOM/config base                              |
|    06 | `source_origin_fixture.csv`  | `op_source_zone`, `op_source_origin`, evidence/verification fixture | warehouse/source schema, supplier            |
|    07 | `recipe_groups.csv`          | `ref_recipe_line_group`                                             | none                                         |
|    08 | `skus.csv`                   | `ref_sku`, `ref_sku_operational_config`                             | UOM/config base                              |
|    09 | `ingredients.csv`            | `ref_ingredient`                                                    | UOM base                                     |
|    10 | `op_supplier_ingredient.csv` | `op_supplier_ingredient`                                            | supplier, ingredient, UOM                    |
|    11 | `g1_recipe_headers.csv`      | `op_production_recipe`                                              | SKU, ingredient                              |
|    12 | `g1_recipe_lines.csv`        | `op_recipe_ingredient`                                              | recipe header, ingredient, recipe group, UOM |
|    13 | `gtin_fixture.csv`           | `op_trade_item`, `op_trade_item_gtin`                               | SKU                                          |
|    14 | `public_trace_policy.csv`    | `op_public_trace_policy`                                            | trace schema                                 |
|    15 | `event_schema_registry.csv`  | `event_schema_registry`                                             | event/outbox foundation                      |
|    16 | `misa_mapping_fixture.csv`   | `misa_mapping`                                                      | integration schema                           |
|    17 | `ui_registry_fixture.csv`    | `ui_screen_registry`, `ui_action_registry`, `ui_menu_item`          | UI schema, permissions                       |
...
| DATA-LOCK-008 | GTIN fixture không phải GTIN production                                                                                                                                             | `gtin_fixture.csv.is_test_fixture = true` cho toàn bộ row; fixture phải được đánh dấu `DEV_TEST_ONLY`; production GTIN import phải dùng `is_test_fixture = false`                                                                                                             |
...
| DATA-LOCK-012 | G1 PILOT seed phải có anchor + normalized ratio scaffold-ready                                                                                                                      | `g1_recipe_headers.csv` có `formula_kind`, `anchor_*`; `g1_recipe_lines.csv` có `formula_kind`, `is_anchor`; mỗi recipe có đúng 1 anchor và `SUM(ratio_percent) ∈ [99.95, 100.05]`. `quantity_per_batch_400` chỉ là source evidence cho owner review trong G1 PILOT seed.     |
| DATA-LOCK-013 | M03A supplier seed không được dùng quyền nội bộ                                                                                                                                     | `roles_permissions.csv` có role `R-SUPPLIER` với namespace `supplier.*`; không seed `raw_intake.*`, `inventory.*`, `recipe.*`, `misa.*`, `trace.internal.*` cho role supplier.                                                                                                |
| DATA-LOCK-014 | Release revoke, QR generate/reprint/void, device callback và supplier admin là action riêng, không dùng chung permission release/print/master-data tổng quát                         | `roles_permissions.csv` có `BATCH_RELEASE_REVOKE`, `QR_GENERATE`, `QR_REPRINT`, `QR_VOID`, `DEVICE_CALLBACK`, `supplier.read`, `supplier.write`, `supplier.create`, `supplier.update`, `supplier.suspend`, `supplier.reactivate`, `supplier.user.manage`.                  |
...
| Supplier                   | `supplier_code`                                                            | upsert fixture only; production supplier list owner-provided                                                |
| Supplier user              | `(supplier_code, username)`                                                | upsert reset-required fixture account; no plaintext password                                                |
| Supplier ingredient        | `(supplier_code, ingredient_code, effective_from)`                         | upsert; only `ACTIVE` + effective mapping can be used by M06 supplier intake                                |
...
| MISA mapping fixture       | `(internal_object_type, internal_object_id, misa_object_type)`             | upsert fixture only; `internal_object_id` là `uuid` khớp `misa_mapping.internal_object_id` (database/03 §3) |
...
| DATA-ASSUMP-004 | `csv/g1_recipe_lines.csv` có 433 data rows, được xác nhận bằng row count và `data/seed_manifest.json`; đây là derived validation count từ source recipe matrix, không suy ra bằng công thức 20 × 21.                         | ACCEPTED_FOR_DEV      | Nếu source recipe CSV đổi, cập nhật `seed_manifest.json`, NFR seed gate và row count cùng lúc. |
| DATA-ASSUMP-005 | `misa_mapping_fixture.csv` là fixture dev/test, không chứa credential hay endpoint thật.                                                                                                                                      | RESOLVED_PF02 | MISA production dùng `MisaSyncOptions.*` + `ClientSecretRef`/`WebhookSecretRef`; không commit secret thật. |
| DATA-ASSUMP-006 | Toàn bộ CSV trong thư mục này phải được đọc/ghi bằng `UTF-8`.                                                                                                                                                                 | ACCEPTED_FOR_DEV      | Seed loader/test runner phải chỉ định encoding để không làm hỏng tên tiếng Việt. |
| DATA-ASSUMP-007 | `batch_size_standard = 400` là số lượng mẻ chuẩn/quantity-per-batch basis, không phải mặc định 400 kg thành phẩm.                                                                                                             | ACCEPTED_FOR_DEV      | API/seed không được suy diễn đơn vị khối lượng từ số 400.                        |

## 8. PF-02 Production Data/Config Closure

| area | production rule | dev/test rule |
|---|---|---|
| GTIN/GS1 | Production GTIN/SSCC rows are owner-provided imports and must use `is_test_fixture=false`; commercial print rejects missing active identifier and any active identifier marked fixture. | `gtin_fixture.csv` stays for tests only and is marked `DEV_TEST_ONLY`. |
| MISA AMIS | Tenant, endpoint, credential and webhook auth are config/secret refs owned by Finance/Integration + DevOps; mapping rows may be imported per environment. | `misa_mapping_fixture.csv` is `DEV_TEST_ONLY`; no literal credential/endpoint. |
| Evidence storage | Production binary evidence is written through storage adapter to company storage server; DB stores object key/URI, hash, MIME, size, scan status and retention class only. | Local filesystem storage is `DEV_TEST_ONLY`; object key format remains production-compatible. |
| Production users/roles | `roles_permissions.csv` is action baseline; real user-role assignment is environment data and can change without code change. | Supplier/user fixtures are `DEV_TEST_ONLY` and reset-required/no-plaintext. |
