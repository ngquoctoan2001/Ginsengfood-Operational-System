...
| Lock | Quy tắc |
| --- | --- |
| `LOCK-G1-ONLY` | G1 là initial operational baseline. Không mô tả research/baseline token lịch sử là công thức vận hành. |
| `LOCK-ONLY-G1-OPS` | Chỉ G1 là baseline vận hành ban đầu; mọi research/baseline token lịch sử không dùng trong seed, production order, material issue, costing, trace, recall, dev handoff hoặc workflow triển khai. |
