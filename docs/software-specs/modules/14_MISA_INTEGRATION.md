...
| BR-M14-005 | Sync logs must not expose token/secret/private data. | `misa_sync_log` | sync log UI/API | SCR-MISA-SYNC | redaction | safe log | TC-M14-SEC-001 |
...
- Test mode/sandbox decision documented if real MISA unavailable.

## 20. Risks

| risk | Impact | Mitigation |
|---|---|---|
| MISA sandbox unavailable | Smoke cannot require synced terminal state | Accept controlled failed/reconciled state with evidence. |
| Mapping owner unclear | Failed sync backlog | Assign Integration Operator/Admin ownership. |
