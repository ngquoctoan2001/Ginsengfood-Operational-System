...
| M06-F07     | Receipt company-create (PURCHASED)  | Procurement tạo receipt `created_by_party = COMPANY` ở `DRAFT/PENDING_SUPPLIER_CONFIRM`; supplier sẽ confirm/decline.                                               | P0       |
| M06-F08     | Receipt supplier-submit (PURCHASED) | Supplier user tạo receipt `created_by_party = SUPPLIER` ở `PENDING_RECEIVE`; bỏ qua bước confirm.                                                                   | P0       |
| M06-F09     | Supplier confirm/decline            | Supplier user confirm (→`PENDING_RECEIVE`) hoặc decline (→`SUPPLIER_DECLINED` final) với reason.                                                                    | P0       |
...
| M06-F13     | Return to supplier                  | Tạo flow EX-RECEIPT-RETURN cho line REJECTED; ghi return_qty + reason; giữ receipt RECEIVED.                                                                        | P0       |
...
| BR-M06-006 | Receipt với `supplier_id + ingredient_id` không có trong `op_supplier_ingredient_allowed` bị reject (HL-SUP-001).                              | receipt + mapping             | raw intake create, supplier submit | SCR-RAW-INTAKES, SCR-SUP-PORTAL-INTAKES | allowlist check            | `SUPPLIER_INGREDIENT_NOT_ALLOWED`                                            | AC-SUP-001    |
| BR-M06-007 | Supplier-submitted receipt edit-locked sau khi supplier `PENDING_RECEIVE` hoặc company-created sau supplier confirm (HL-SUP-002).              | receipt                       | receipt update                     | SCR-SUP-PORTAL-INTAKES                  | state guard                | `SUPPLIER_EDIT_LOCKED_AFTER_CONFIRMED`                                       | AC-SUP-002    |
...
| BR-M06-009 | Receipt sau RECEIVED không sửa quantity/ingredient; chỉ accept/reject/return/close (HL-SUP-004).                                               | receipt + line                | line action APIs                   | SCR-RAW-RECEIVE, SCR-RAW-LINE-QC        | state guard                | `RECEIPT_LOCKED_AFTER_RECEIVE`                                               | AC-SUP-004    |
...
Hai trục độc lập: `supplier_collaboration_status` (axis A) và `raw_receipt_status` (axis B). Trục A chỉ active khi `procurement_type = PURCHASED`; `SELF_GROWN` bypass axis A (giữ `NOT_APPLICABLE`).

#### 11.2.1 Axis A — supplier_collaboration_status

```mermaid
stateDiagram-v2
    [*] --> NOT_APPLICABLE: SELF_GROWN
    [*] --> PENDING_SUPPLIER_CONFIRM: company-create + PURCHASED
    [*] --> SUPPLIER_SUBMITTED: supplier-submit
    PENDING_SUPPLIER_CONFIRM --> SUPPLIER_CONFIRMED: supplier confirm
    PENDING_SUPPLIER_CONFIRM --> SUPPLIER_DECLINED: supplier decline (final)
    SUPPLIER_SUBMITTED --> SUPPLIER_CONFIRMED: auto on submit
    SUPPLIER_CONFIRMED --> COMPANY_ACKNOWLEDGED: warehouse acknowledge ready to receive
    COMPANY_ACKNOWLEDGED --> COLLABORATION_CLOSED: receipt CLOSED
    SUPPLIER_DECLINED --> [*]
    NOT_APPLICABLE --> [*]
...
stateDiagram-v2
    ...
    [*] --> PENDING_RECEIVE: supplier-submit hoặc supplier confirm
    DRAFT --> PENDING_SUPPLIER_CONFIRM: send to supplier
    PENDING_SUPPLIER_CONFIRM --> PENDING_RECEIVE: supplier confirm
    PENDING_SUPPLIER_CONFIRM --> CANCELLED: supplier decline
    PENDING_RECEIVE --> RECEIVED: warehouse confirm received
    RECEIVED --> PARTIALLY_ACCEPTED: some lines accepted, some rejected/returned
    RECEIVED --> ACCEPTED: all lines accepted
    RECEIVED --> REJECTED: all lines rejected
    PARTIALLY_ACCEPTED --> CLOSED: close receipt
    ACCEPTED --> CLOSED: close receipt
    REJECTED --> CLOSED: close receipt
    DRAFT --> CANCELLED: cancel before send
    CANCELLED --> [*]
    CLOSED --> [*]
```
...
| ---------------- | ---------------- | ----------------------------------------- | ---------------------------------- | ---------------------------------------------------------------------- |
| COMPANY          | PURCHASED        | `PENDING_SUPPLIER_CONFIRM`                | `DRAFT → PENDING_SUPPLIER_CONFIRM` | `(COLLABORATION_CLOSED, CLOSED)` hoặc `(SUPPLIER_DECLINED, CANCELLED)` |
| SUPPLIER         | PURCHASED        | `SUPPLIER_SUBMITTED → SUPPLIER_CONFIRMED` | `PENDING_RECEIVE`                  | `(COLLABORATION_CLOSED, CLOSED)`                                       |
| COMPANY          | SELF_GROWN       | `NOT_APPLICABLE`                          | `DRAFT → PENDING_RECEIVE`          | `(NOT_APPLICABLE, CLOSED)`                                             |
...
| VAL-M06-008   | Edit receipt sau supplier confirm/`PENDING_RECEIVE` bị chặn (trừ override)            | `SUPPLIER_EDIT_LOCKED_AFTER_CONFIRMED`                                            | Yes        |
| VAL-M06-009   | Decline final, không reopen                                                           | `SUPPLIER_DECLINED_BLOCKED`                                                       | Yes        |
| VAL-M06-010   | Edit quantity/ingredient sau RECEIVED bị chặn                                         | `RECEIPT_LOCKED_AFTER_RECEIVE`                                                    | Yes        |
...
| EX-RECEIPT-RETURN     | Line REJECTED → ghi `returned_quantity` + reason → gửi trả supplier  | Receipt giữ trạng thái RECEIVED/PARTIALLY_ACCEPTED, không sửa quantity gốc. Xem `workflows/07_EXCEPTION_FLOWS.md`. |
...
| AC-SUP-004    | Edit quantity/ingredient sau RECEIVED        | Reject `RECEIPT_LOCKED_AFTER_RECEIVE`                         | P0       |
