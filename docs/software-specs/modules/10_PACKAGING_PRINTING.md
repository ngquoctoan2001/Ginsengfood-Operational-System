...
Nguồn dữ liệu packaging hierarchy: `op_trade_item.units_per_box`, `op_trade_item.boxes_per_carton`, `op_trade_item.carton_enabled`. Identifier (GTIN_13/GTIN_14/SSCC/INTERNAL_BARCODE/TEST_FIXTURE) lấy từ `op_trade_item_gtin.identifier_type` + `identifier_value`.
...
| `identifier_type`  | enum `GTIN_13`/`GTIN_14`/`SSCC`/`INTERNAL_BARCODE`/`TEST_FIXTURE` | Khi tạo identifier          | —       | PACKET/BOX dùng `GTIN_13` hoặc `GTIN_14`; CARTON dùng `GTIN_14` hoặc `SSCC`; dev dùng `TEST_FIXTURE`.                         |
| `identifier_value` | text                                                              | Khi tạo identifier          | —       | UNIQUE per `(identifier_type, identifier_value)` khi `status = ACTIVE`.                                                       |
| `is_test_fixture`  | bool                                                              | Yes                         | false   | Bật cho `TEST_FIXTURE` ở dev/test. Production print phải fail nếu `is_test_fixture = true`.                                   |
...
| ---------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Printer model/callback unspecified             | Incomplete automation                       | Define adapter/callback in CODE12 owner decision (ADR-018).                                                                                                                                                                                                                                |
