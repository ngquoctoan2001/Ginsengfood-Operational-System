...
| M04-F03 | Recipe draft | Tạo recipe version draft theo SKU. | P0 |
| M04-F04 | Recipe lines | Quản lý line theo 4 group chuẩn, quantity, UOM, prep note, usage role. | P0 |
| M04-F05 | Approval/activation | Submit, approve, activate, retire recipe version. | P0 |
| M04-F06 | Snapshot provider | Cung cấp active approved recipe lines cho production order snapshot; thiếu active recipe phải trả `ACTIVE_RECIPE_NOT_FOUND`. | P0 |
| M04-F07 | Future version support | Hỗ trợ G2/G3/... không sửa lịch sử PO cũ. | P0 |

## 5. Business Rules

| rule_id | Rule | Affected data | Affected API | Affected UI | Validation | Exception | Test |
|---|---|---|---|---|---|---|---|
| BR-M04-001 | G1 là initial operational baseline cho go-live. | `op_production_recipe` | `/api/admin/recipes/*` | SCR-RECIPE | version/status/effective date | Non-operational version blocked | TC-M04-REC-004 |
...
| BR-M04-003 | Chỉ một active operational recipe cho mỗi SKU tại một thời điểm. | `op_production_recipe` | activate endpoint | SCR-RECIPE | partial unique active per SKU | `ACTIVE_RECIPE_CONFLICT` | TC-M04-REC-006 |
...
| BR-M04-008 | Production snapshot phải capture `formula_version`, `recipe_line_group_code`, ingredient code/display name, quantity per batch 400, UOM, prep note và usage role. | recipe snapshot tables | M07 create PO | SCR-PROD-ORDER-DETAIL | snapshot completeness | `SNAPSHOT_INCOMPLETE` | TC-M04-SNAP-001 |
...
| `op_production_recipe` | versioned master | Recipe header/version/status. | M04 | Includes `formula_version`; G1 baseline active, future G2/G3 supported, G0 not active operational. |
| `op_recipe_ingredient` | versioned detail | Recipe lines. | M04 | Includes `recipe_line_group_code`, quantity per batch, UOM, prep note, usage role. |
...
| VAL-M04-004 | Quantity per batch > 0 | `VALIDATION_FAILED` | Yes |
| VAL-M04-005 | Activate only approved/effective recipe | `RECIPE_NOT_APPROVED` | Yes |
| VAL-M04-006 | Only one active operational recipe per SKU | `ACTIVE_RECIPE_CONFLICT` | Yes |
...
| TC-M04-SNAP-001 | Snapshot provider returns full G1 line metadata | Snapshot includes formula version and 4 group codes | P0 |
...
- G1 recipe baseline active per go-live SKU.
- G0 is not seeded or activated as an operational recipe.
- Recipe lines use exactly 4 groups.
- Snapshot provider returns `formula_version` and `recipe_line_group_code` for every line.
- Approval/activation/retire lifecycle works.
- Production order can snapshot recipe and keep history immutable.
- Negative tests block non-operational version and invalid groups.
