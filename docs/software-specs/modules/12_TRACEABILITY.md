...
| BR-M12-003 | QR `VOID`/`FAILED` returns safe public invalid/void state. | QR status/projection | public trace API | SCR-PUBLIC-TRACE | QR lifecycle check | safe response | TC-OP-QR-001 |
...
| GET | `/api/public/trace/{qrCode}` | Public trace resolve | N/A | No | N/A | `PublicTraceResponse` | TC-M12-PTRACE-002 |
...
| WF-M12-PUBLIC | QR printed/public lookup | Resolve QR -> apply policy -> return safe fields | PublicTraceResponse | `api/03_API_REQUEST_RESPONSE_SPEC.md` |
...
sequenceDiagram
    ...
    PublicAPI-->>Public: PublicTraceResponse
```
...
| API | search params, QR code | InternalTraceResponse, PublicTraceResponse |
