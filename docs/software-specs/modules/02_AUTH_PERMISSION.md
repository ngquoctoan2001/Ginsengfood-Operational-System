...
- Role/action permission seed covers all P0 module actions, including `RAW_LOT_MARK_READY`, `BATCH_RELEASE_REVOKE`, `QR_REPRINT`, `RECALL_CAPA_EVIDENCE_ADD`, `ACCOUNTING_DOCUMENT_POST`, `BREAK_GLASS_ACTIVATE` and `BREAK_GLASS_DEACTIVATE`.
- Role seed includes external role `R-SUPPLIER` (Supplier Portal scope) plus permission codes `supplier.read` and `supplier.write` mapped per M03A; Supplier Portal users must only receive `R-SUPPLIER` and never any internal role.
