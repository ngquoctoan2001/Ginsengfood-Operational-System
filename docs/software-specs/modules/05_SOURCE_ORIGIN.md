...
| M05-F03a    | Upload media evidence (photo/video) | Upload file ảnh/video làm evidence cho source origin verification; FE upload qua storage adapter rồi gửi metadata (`evidenceType`, `evidenceRef`, `mimeType`, `fileSizeBytes`, `originalFilename`, `evidenceHash`) qua `POST /api/admin/source-origins/{id}/evidence`. Dev/test dùng local filesystem storage; production dùng server lưu trữ của công ty qua cấu hình. MIME allowlist: `image/jpeg`,`image/png`,`image/webp` (≤10MB); `video/mp4`,`video/quicktime` (≤100MB). Evidence phải scan sạch trước khi verify. | P0       |
...
| BR-M05-006 | Evidence upload chỉ chấp nhận MIME trong allowlist (`image/jpeg`,`image/png`,`image/webp`,`video/mp4`,`video/quicktime`) và size cap (image ≤10MB, video ≤100MB). Binary lưu qua storage adapter; dev/test dùng local filesystem storage, production dùng server lưu trữ của công ty qua cấu hình; DB chỉ metadata (`evidence_uri`, `evidence_hash`, `mime_type`, `file_size_bytes`, `original_filename`, `scan_status`). | `op_source_origin_evidence`                     | `POST .../evidence`  | SCR-SOURCE-ORIGINS       | MIME/size check          | `EVIDENCE_MIME_NOT_ALLOWED`, `EVIDENCE_FILE_TOO_LARGE` | TC-M05-SRC-002 |
| BR-M05-007 | Source origin `verify` yêu cầu ít nhất 1 evidence hợp lệ (`scan_status = CLEAN`, hash khớp, MIME hợp lệ) trước khi chuyển `SUBMITTED → VERIFIED`. Evidence `PENDING_SCAN`, `SCAN_FAILED` hoặc `INFECTED` không được tính là hợp lệ.                                                                                                                                                                | `op_source_origin`, `op_source_origin_evidence` | verify API           | SCR-SOURCE-ORIGINS       | evidence existence + scan check | `EVIDENCE_REQUIRED`, `EVIDENCE_SCAN_PENDING`, `EVIDENCE_SCAN_FAILED`, `EVIDENCE_MALWARE_DETECTED` | TC-UI-SRC-002  |
...
| `op_source_origin_evidence`     | history/file-ref | Evidence references.              | M05       | Store file object key/URI, checksum, MIME type, size bytes, original filename, `scan_status` và retention class; binary không lưu blob trong DB. Dev/test local filesystem storage; production company storage server configured by DevOps. Cột bắt buộc: `evidence_type` (enum), `evidence_uri`, `mime_type` (allowlist), `file_size_bytes` (≤10MB image / ≤100MB video), `scan_status`. |
...
| VAL-M05-007   | Evidence MIME phải nằm trong allowlist                          | `EVIDENCE_MIME_NOT_ALLOWED`     | Yes      |
| VAL-M05-008   | Evidence file size vượt cap theo MIME (image 10MB, video 100MB) | `EVIDENCE_FILE_TOO_LARGE`       | Yes      |
| VAL-M05-009   | Verify yêu cầu ít nhất 1 evidence hợp lệ đã upload thành công và `scan_status = CLEAN` | `EVIDENCE_REQUIRED`             | Yes      |
| VAL-M05-010   | Evidence chưa scan xong không được dùng để verify               | `EVIDENCE_SCAN_PENDING`         | Yes      |
| VAL-M05-011   | Evidence scan lỗi hoặc phát hiện malware bị reject/block verify | `EVIDENCE_SCAN_FAILED`, `EVIDENCE_MALWARE_DETECTED` | Yes      |
...
| TC-UI-SRC-003 | Verify source origin with pending/failed/malware evidence scan | Verification blocked              | P0       |
