...
| M13 | `modules/13_RECALL.md` | Recall | Incident, recall case, hold, sale lock, exposure, recovery, disposition, CAPA. | `SRC-FILE01`, `SRC-FILE03` |
...
M04 SKU Ingredient Recipe
  ↓
M05 Source Origin → M06 Raw Material → M08 Material Issue Receipt
                                      ↓
                                  M07 Production
                                      ↓
...
| M06 | `op_raw_material_receipt`, `op_raw_material_receipt_item`, `op_raw_material_lot`, `op_raw_material_qc_inspection` |
...
| M13 | `op_incident_case`, `op_recall_case`, `op_recall_case_batch`, `op_batch_hold_registry`, `op_sale_lock_registry`, `op_recall_recovery_item`, `op_recall_disposition_record`, `op_recall_capa` |
...
| M06 | `/api/admin/raw-material/intakes/*`, `/api/admin/raw-material/lots/*`, `/api/admin/raw-material/qc/*` |
...
| M06 | Raw material intake, raw material QC, raw lot |
...
| M13 | Incident, recall case, recovery, disposition, CAPA |
...
| M06 | M08 | Chỉ raw material lot `QC_PASS` mới được issue. |
