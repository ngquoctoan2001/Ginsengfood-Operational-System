...
| M03A | M02 | Supplier user dùng `user_type = SUPPLIER_USER` + role `R-SUPPLIER`; namespace `supplier.*` tách hẳn, không kế thừa `raw_intake.read` của staff role (HL-SUP-007, HL-SUP-008).                                        |
