...
| Trạng thái lot nguyên liệu   | `lot_status`                    | Lifecycle persisted của raw material lot: `CREATED`, `IN_QC`, `ON_HOLD`, `REJECTED`, `READY_FOR_PRODUCTION`, `CONSUMED`, `EXPIRED`, `QUARANTINED`.                 |
| Trạng thái QC lot            | `lot_qc_status`                 | Trạng thái QC của raw material lot; khởi tạo `PENDING_QC`.                                                                                                         |
| Operational hold             | `ON_HOLD`                       | Tạm giữ nội bộ để điều tra/vận hành; block material issue, có thể release về luồng hợp lệ sau review/audit.                                                        |
| Quarantine                   | `QUARANTINED`                   | Cách ly safety/legal mạnh hơn `ON_HOLD`; block material issue mặc định và cần disposition hoặc quarantine-release approval riêng.                                    |
| Supplier user                | `SUPPLIER_USER`                 | External user thuộc nhà cung cấp (`user_type = SUPPLIER_USER`), gắn `supplier_id`, dùng role `R-SUPPLIER`.                                                         |
...
| Role baseline count          | `auth_role` seed count          | Seed role baseline gồm 16 role nội bộ trong admin/operational matrix + 1 external role `R-SUPPLIER` cho Supplier Portal; tổng cộng 17 `auth_role` baseline.        |
...
| Recipe status          | `DRAFT`, `PENDING_APPROVAL`, `APPROVED`, `APPROVED_SEED_BASELINE`, `ACTIVE_OPERATIONAL`, `RETIRED`, `REJECTED` | Trạng thái quản trị version công thức; production order chỉ dùng `ACTIVE_OPERATIONAL`. Active uniqueness là per `(sku_id, formula_kind)`, không phải per SKU. |
