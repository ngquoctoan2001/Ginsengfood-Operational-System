...
- **Quyết định owner**: công ty đã có GTIN thật nhưng chưa cấp cho dev, dev cấu hình fake fixture cho cả 2 cấp đóng gói:
  ...
  - Đánh dấu fixture `DEV_TEST_ONLY` và `is_test_fixture=true` để khi có GTIN thật thì swap không gây drift; production rows phải `is_test_fixture=false`.
...
| OD-03                          | Production GTIN/GS1 cho 20 SKU + packaging level               | RESOLVED_FINAL_PF02 → dev/test fake fixture `DEV_TEST_ONLY` cho BOX/CARTON; production import GTIN/SSCC thật với `is_test_fixture=false`; PACKET không QR/trace standalone.                                                                                                                                                                                                                   |
| OD-04                          | MISA endpoint/credential production                            | RESOLVED_FINAL_PF02_WITH_SECRET_REFS → MISA AMIS, `DryRun`/`Production` mode, tenant/endpoint/credential qua config/secret refs, không commit secret thật.                                                                                                                                                                                                                                  |
...
### C.2 — ~~Còn OPEN~~ → Đã đóng (PF-01, 2026-05-03)

> **Trạng thái:** Tất cả OD bên dưới đã được owner chốt trong `docs/v2-decisions/OD-DEFAULTS-2026-05-03.md` (PF-01, 2026-05-03). Xem §C.8 bên dưới để biết trạng thái cuối của từng OD. Không còn OD nào block CODE01–CODE17 scaffold.

| ID    | Điểm cần quyết                                                                                        | Severity | CODE chặn            | Trạng thái PF-01            |
| ----- | ----------------------------------------------------------------------------------------------------- | -------- | -------------------- | --------------------------- |
| OD-11 | Trace query SLA technical: target latency?                                                            | MEDIUM   | CODE07               | RESOLVED_FINAL              |
| OD-12 | Backup/DR target: RPO, RTO?                                                                           | HIGH     | CODE16               | RESOLVED_FINAL              |
| OD-13 | Audit log retention: bao lâu?                                                                         | HIGH     | CODE16               | RESOLVED_FINAL              |
| OD-14 | Public trace có cần multi-language không?                                                             | LOW      | CODE07               | RESOLVED_FINAL              |
| OD-17 | Production printer model + driver chính thức                                                          | MEDIUM   | CODE12               | RESOLVED_FINAL_PF02_WITH_DEVICE_REFS |
| OD-20 | MISA AMIS tenant/credential/endpoint thật cho production                                              | HIGH     | CODE13/CODE17        | RESOLVED_FINAL_PF02_WITH_SECRET_REFS |
| OD-21 | PWA task taxonomy và endpoint inbox `/api/admin/tasks/my`                                             | MEDIUM   | CODE11               | RESOLVED_FINAL              |
| OD-22 | Các mutation endpoint UI phụ: UOM write, raw lot hold/release, process command, screen registry write | MEDIUM   | CODE09/CODE10/CODE11 | RESOLVED_FINAL              |
...
| OD-17 | Production printer driver/model final cho CODE12.               | PF-02 baseline: HTTP/ZPL-compatible adapter, HMAC-SHA256 callback, physical model/IP/driver là device registry/config refs. |
| OD-20 | Real MISA sync enablement cho production.                       | PF-02 baseline: `DryRun`/`Production` mode; tenant/endpoint/credential qua config/secret refs có owner; không hard-code secret. |
| OD-21 | PWA task inbox contract final.                                  | Dùng 15 task type taxonomy default cho `/api/admin/tasks/my`; command vẫn dùng idempotency và task-specific permission. |
...
### C.8 — PF-01 Owner Decision Finalization 2026-05-03

> Cập nhật PF-02 ngày 2026-05-03: production data/config closure đã chuyển OD-17 và OD-20 từ accepted-risk sang final bằng device/config/secret refs có owner. Các giá trị hạ tầng thật được bind runtime qua DI/config table/secret manager, không đổi schema/API và không commit secret thật. Không còn OD nào block CODE00 → CODE17 scaffold hoặc READY_FOR_PRODUCTION_FREEZE; go-live chỉ cần nạp giá trị production theo refs đã khóa.

#### C.8.a — Batch 1 (8 OD)

| OD    | Trạng thái cuối PF-01       | Owner/date         | Impact production freeze                                                                                                                                |
| ----- | --------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OD-11 | RESOLVED_FINAL              | owner / 2026-05-03 | Trace SLA p95 < 500ms public, < 1.5s genealogy depth ≤ 5; runtime config qua `op_trace_sla_config`.                                                     |
| OD-12 | RESOLVED_FINAL              | owner / 2026-05-03 | RPO 15 phút / RTO 4 giờ DB; adapter `IBackupAdapter`/`IRestoreAdapter`; evidence backup theo storage adapter được cấu hình.                             |
| OD-13 | RESOLVED_FINAL              | owner / 2026-05-03 | Retention 7 năm operational, 10 năm recall, 90 ngày outbox; runtime swap qua `op_retention_policy`.                                                     |
| OD-14 | RESOLVED_FINAL              | owner / 2026-05-03 | MVP single-language `vi`; schema i18n-ready (`*_i18n` JSONB).                                                                                           |
| OD-17 | RESOLVED_FINAL_PF02_WITH_DEVICE_REFS | owner / 2026-05-03 | Build `IPrinterAdapter`, HTTP/ZPL-compatible payload + HMAC-SHA256 callback; physical model/IP/driver là device registry/config refs do Packaging Ops/DevOps sở hữu. |
| OD-20 | RESOLVED_FINAL_PF02_WITH_SECRET_REFS | owner / 2026-05-03 | Build MISA AMIS `DryRun`/`Production` mode; production sync dùng `MisaSyncOptions.*` + `ClientSecretRef`/`WebhookSecretRef`, không commit secret thật. |
| OD-21 | RESOLVED_FINAL              | owner / 2026-05-03 | 15 task type taxonomy default cho `/api/admin/tasks/my`.                                                                                                |
| OD-22 | RESOLVED_FINAL              | owner / 2026-05-03 | 5 route family default theo REST + action sub-resource.                                                                                                 |

#### C.8.b — Batch 2 freeze review (15 OD)

| OD                         | Trạng thái cuối PF-01 | Owner/date         | Impact production freeze                                                                                                                                                        |
| -------------------------- | --------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OD-G2-001                  | RESOLVED_FINAL        | owner / 2026-05-03 | Tolerance ±2%; warning ≤ 10%; block > 10% + CAPA. Configurable qua `op_formula_g2_header.tolerance_pct`.                                                                        |
| OD-FORMULA-PICK-001        | RESOLVED_FINAL        | owner / 2026-05-03 | Rule ưu tiên FIXED khi tie; planner override với `override_reason` audit. Config qua `preferred_formula_kind` per SKU.                                                          |
| OD-EVIDENCE-SCAN-001       | RESOLVED_FINAL        | owner / 2026-05-03 | ClamAV default + `IEvidenceScanner` abstraction; swap WindowsDefender/AzureDefender/Sophos qua DI.                                                                              |
| OD-INVENTORY-ALLOC-001     | RESOLVED_FINAL        | owner / 2026-05-03 | FEFO default ngành thực phẩm; configurable per-SKU qua `op_sku_operational_config.allocation_strategy`.                                                                         |
| OD-WAREHOUSE-3RD-001       | RESOLVED_FINAL        | owner / 2026-05-03 | Giữ 2 `warehouse_type` chính; dùng `op_warehouse_zone` cho quarantine/return/recall hold.                                                                                       |
| OD-SUP-AUTH-001            | RESOLVED_FINAL        | owner / 2026-05-03 | Supplier portal email/password; TOTP configurable, session idle 30 phút, absolute 8 giờ, lockout 5 failed/15 phút.                                                              |
| OD-SUP-EVIDENCE-RETRY-001  | RESOLVED_FINAL        | owner / 2026-05-03 | Replacement upload append-only khi `SCAN_FAILED` hoặc `INFECTED`; không xóa evidence trail cũ.                                                                                  |
| OD-SUP-FEEDBACK-ENUM-001   | RESOLVED_FINAL        | owner / 2026-05-03 | Feedback enum baseline: `QUALITY_ISSUE`, `DELIVERY_LATE`, `DELIVERY_EARLY`, `QUANTITY_VARIANCE`, `DOCUMENTATION_INCOMPLETE`, `PACKAGING_DAMAGE`, `TEMPERATURE_BREACH`, `OTHER`. |
| OD-SUP-CONFIRM-TIMEOUT-001 | RESOLVED_FINAL        | owner / 2026-05-03 | Supplier confirmation: alert T+24h, auto-cancel T+72h, configurable worker.                                                                                                     |
| OD-PACKAGING-DEFAULT-001   | RESOLVED_FINAL        | owner / 2026-05-03 | Default 3-tier PACKET → BOX → CARTON; 4 PACKET/BOX và 12 BOX/CARTON, SKU override qua packaging config.                                                                         |
| OD-CAPA-MODEL-001          | RESOLVED_FINAL        | owner / 2026-05-03 | `op_recall_capa` schema với owner, due_date, close_gate, evidence_count, status; evidence must be `CLEAN` before close.                                                         |
| OD-MISA-DOC-OWNER-001      | RESOLVED_FINAL        | owner / 2026-05-03 | M08 emits operational facts/outbox; M14 owns MISA document payload/post lifecycle.                                                                                              |
| OD-ADJUSTMENT-APPROVAL-001 | RESOLVED_FINAL        | owner / 2026-05-03 | Adjustment tier policy: <1% auto record, 1-5% manager approval, >5% two-person approval.                                                                                        |
| OD-WH-CORRECT-IDEMPO-001   | RESOLVED_FINAL        | owner / 2026-05-03 | Warehouse receipt correction append-only with `correction_seq`; idempotency only deduplicates the same command.                                                                 |
| OD-RATE-LIMIT-SCOPE-001    | RESOLVED_FINAL        | owner / 2026-05-03 | Rate limit required for public trace, auth, supplier, MISA callback, printer/device callback, admin/PWA mutation.                                                               |

#### C.8.c — 3 OD business critical owner FINAL

| ID                      | Trạng thái cuối PF-01 | Owner/date         | Impact production freeze                                                                                                                                                                      |
| ----------------------- | --------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OD-EVIDENCE-STORAGE-001 | RESOLVED_FINAL        | owner / 2026-05-03 | Evidence lưu production trên server công ty qua `COMPANY_SERVER` storage profile/adapter; dev/test dùng `LocalFileSystemEvidenceAdapter` trên máy local; DB chỉ lưu metadata/object key/hash. |
| OD-NOTIFY-OWNERSHIP-001 | RESOLVED_FINAL        | owner / 2026-05-03 | Operational chỉ tạo notification job/outbox và `notification_job_id`; hệ thống bán hàng/external notification service chịu trách nhiệm delivery/SLA kênh gửi.                                 |
| OD-PACKET-TRACE-001     | RESOLVED_FINAL        | owner / 2026-05-03 | PACKET không in QR vì luôn nằm trong BOX; public trace entry point là BOX/CARTON QR, PACKET inherit batch/trace từ parent packaging.                                                          |

#### C.8.d — Seed canonical patch (2026-05-03)

Đồng thời thêm vào seed canonical để mapping permission/role spec UPPER → dotted convention OD-SEED-NAMING-001:

| Spec UPPER             | Seed dotted                                                                 | File                            |
| ---------------------- | --------------------------------------------------------------------------- | ------------------------------- |
| `R-SUPPLIER` (role)    | `R-SUPPLIER` (UPPER giữ nguyên cho external scope)                          | `docs/seeds/01_roles.sql`       |
| `RAW_LOT_MARK_READY`   | `raw-material.lot.mark-ready`                                               | `docs/seeds/02_permissions.sql` |
| `BATCH_RELEASE_REVOKE` | `qc-release.batch-release.revoke`                                           | `docs/seeds/02_permissions.sql` |
| `QR_REPRINT`           | `traceability.qr-registry.reprint` + `packaging.print.reprint` (đã tồn tại) | `docs/seeds/02_permissions.sql` |
...
| HS-04 | Trace query latency target                                                                      | PF-01 đã chốt qua OD-11; NFR/query metric dùng baseline đã freeze.                            |
| HS-05 | Audit log retention rule                                                                        | PF-02 đã chốt qua OD-13; compliance dùng baseline retention/archive đã freeze.                |
...
Ghi chú PF-01 2026-05-03: owner finalization đã cập nhật §C.8 sang trạng thái cuối `RESOLVED_FINAL` hoặc `DEFERRED_WITH_ACCEPTED_RISK`; chốt FINAL cho evidence storage trên server công ty + local dev/test, notification outbox-only do hệ thống bán hàng deliver, PACKET không QR và inherit BOX/CARTON trace.
...
| 2026-05-03 | Turn 6 directive "giải quyết triệt để": đăng ký §C.8 với toàn bộ 8 OD Batch 1 + 15 OD Batch 2 + 3 OD trước đây BLOCKING (OD-EVIDENCE-STORAGE-001 / OD-NOTIFY-OWNERSHIP-001 / OD-PACKET-TRACE-001) chuyển sang `OWNER_ACCEPTED_AS_DEFAULT_2026-05-03` với provisional adapter/boundary/policy làm production-acceptable. Runtime swap qua DI binding hoặc `op_*_config`. Thêm vào seed canonical: role `R-SUPPLIER` (file `docs/seeds/01_roles.sql`) + 2 permission code `raw-material.lot.mark-ready` + `qc-release.batch-release.revoke` (file `docs/seeds/02_permissions.sql`); mapping `QR_REPRINT` đã được phủ bởi `traceability.qr-registry.reprint` + `packaging.print.reprint` đang tồn tại. Không còn OD nào block CODE00 → CODE17. | Codex            |
