...
Đầu ra: gap map, role drift report, first gap, cập nhật tiến độ.
```
...
Đầu ra: registry plan, role decision list, seed/API/UI/test scope, cập nhật tiến độ.
```
...
Đầu ra: service/API/seed/tests, cập nhật tiến độ.
```
...
Đầu ra: UI files, API client/types, UI tests, cập nhật tiến độ.
```
...
Đầu ra: verdict, findings, cập nhật tiến độ.
```
