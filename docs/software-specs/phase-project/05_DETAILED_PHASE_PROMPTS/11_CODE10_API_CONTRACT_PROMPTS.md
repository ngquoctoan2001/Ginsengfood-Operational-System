...
Đầu ra: route/DTO/error/idempotency/pagination drift report, first gap, cập nhật tiến độ.
```
...
Đầu ra: API/FE sync evidence, tests, cập nhật tiến độ.
```
...
Đầu ra: DB/backend/API tests, endpoint list, cập nhật tiến độ.
```
...
Đầu ra: API helpers, route updates, frontend client/types, tests, cập nhật tiến độ.
```
...
Đầu ra: sync report, changed files, tests, cập nhật tiến độ.
```
...
Đầu ra: ACCEPT/NEEDS_FIX/REJECT, findings, commands, cập nhật tiến độ.
```
