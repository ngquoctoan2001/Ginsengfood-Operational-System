...
Đầu ra: device boundary gap map, direct DB bypass risk, blocker list, cập nhật tiến độ.
```
...
Đầu ra: adapter contract, callback/log model, tests, cập nhật tiến độ.
```
...
Đầu ra: API/service/tests, cập nhật tiến độ.
```
...
Đầu ra: device list, heartbeat status, print failure logs, retry/reprint links, tests, cập nhật tiến độ.
```
...
Đầu ra: verdict, blockers, cập nhật tiến độ.
```
