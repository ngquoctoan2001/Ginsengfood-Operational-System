...
| Recall | Recall | `/admin/recall/*` | Sự cố, phân tích ảnh hưởng, hold/sale lock, phục hồi, CAPA. |
...
| M05 Source Origin | Source Zones, Source Origins | Source Origin Detail | Verify/reject source origin | Raw intake chọn source origin verified |
| M06 Raw Material | Raw Intakes, Raw Lots, Lot Readiness Queue | Intake Detail, Lot Detail, Readiness Detail | Receive, QC handoff, mark `READY_FOR_PRODUCTION`, hold, release hold, cancel | PWA scan raw lot; PWA action `mark_ready` nếu có permission |
...
| M13 Recall | Incidents, Recall Cases | Recall Detail, Impact Analysis | Hold, sale lock, recover, close, CAPA | Không áp dụng |
